...
export default class Twitterbot {
	...
	drainQueue() {
		...
		if(tweetQueue.length < 1) {
			console.log('No tweets scheduled, will try later');
	...
	prepareReply(message) {
		let text = message.text.replace(/^@[@\w\s_]+:?\s/ig, '');
		...
		getIntent(message.text).then((o) => {
			if(o.intent.intent !== 'None') {
				reply(replyId, from, handleIntent(message.text, o));
	...
	listen() {
		...
		this.postTweet();
		// schedule a tweet for every five hours
		setInterval(this.postTweet.bind(this), 18000000);
	}
