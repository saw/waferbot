...
export default class Twitterbot {
	...
	handleBotMention(tweet) {
		if(this.handledMentions[tweet.id_str]) {
			return;
		}
		if(tweet.user.followers_count < 1000 && Math.random() > 0.9) {
	...
	tweet(tweetObj) {
		...
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				// console.log('would have sent', tweetObj.message);
				resolve({id_str: 'FAKE', text:tweetObj.message })
			})
