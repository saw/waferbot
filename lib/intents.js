...
const URL = `https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/${APP_ID}?subscription-key=${SUBSCRIPTION_KEY}&timezoneOffset=10.0`;
...
const intentHandlers = {
	...
	'thanks': function(text, intent) {
		return randomItem(['course', "you're welcome", "😀"]);
	},
	...
	'weather': function(text, intent) {
		return randomItem([
			'I think it will be cold. Or hot',
			"I'm not fucking siri, ask someone who knows",
			"Frost, heat, air, water"
		])
	}
...
function handleEntity(text, entities) {
	...
	entities.forEach((entity) => {
		if(entity.type === 'builtin.encyclopedia.people.person') {
			retMessage = randomItem(
				[
					`${entity.entity} is a bit of a cunt`,
					`Such a twat.`, 'huh', `oh i like ${entity.entity}`,
					`never met them`,
					`oh ${entity.entity}! I should interview them!`,
					"who's that?",
					`oh right, forgot about ${entity.entity}, how are they doing?`,
					`hrmm`,
					`${entity.entity}??`,
					randomItem(positiveOpinions),
					"gah",
					`wow I have not seen ${entity.entity} since I worked at flickr`,
					`whaaat?`,
					randomItem(queryResponses)
				]);
		} 
